...
public function orders_not_approval(Request $request)
{
    ...
    public function show(Order $order)
    {
        ...
        $organizationTypes = OrganizationType::get();
        $organizationactivities = Organizationactive::get();
        $salary             = $order['orderDetailsCar']['salary'] ?? 0;
        ...
        $commitment         = $order['orderDetailsCar']['commitments']??0;
        if ($commitment > $salary)
        {
            $approve_amount = 0;
        } else
        {
            $approve_amount = ((($salary + ($order['orderDetailsCar']['having_loan_support_price'] ?? 0)) * ($precentage_approve / 100)) - ($commitment ?? 0));
