        ...
        <div class="flex-lg-row-fluid ms-lg-7 ms-xl-10">
            <div class="card">
                ...
                <div class="card-body p-0">
                    ...
                    <!--begin::Body-->
                    <form action="{{ back()->getTargetUrl()}}" class="form" method="get"
                        data-redirection-url="{{ route('dashboard.chats.index') }}">
                        @csrf
                        <div class="d-block">
                            <div class="d-flex align-items-center border-bottom px-8 min-h-50px">
                                <!--begin::Label-->
                                <div class="text-gray-900 fw-bold w-75px">{{ __('From') . ' ' . ':' }}</div>
                                <!--end::Label-->
                                <div>
                                    <h4>{{ $massage->sender->name }}</h4>
                                </div>
                            ...
                            <!--begin::Subject-->
                            <div class="d-flex align-items-center border-bottom px-8 min-h-50px">
                                <div class="text-gray-900 fw-bold w-75px">{{ __('Subject') . ' ' . ':' }}</div>

                                <input class="form-control form-control-transparent border-0 px-8 min-h-45px"
                                    name="compose_subject" value="{{ $massage->subject ?? '' }}"
                                    placeholder="{{ __('Subject') }}" disabled>
                            </div>
                            <p class="invalid-feedback text-center" id="compose_subject"></p>

                        <div id="kt_inbox_form_editor" class="bg-transparent border-0 h-350px px-3 ql-snow">

                                <textarea class="form-control border-0 h-350px px-3" name="compose_contain" rows="5" disabled readonly>{{ $massage->content ?? '' }}</textarea>
